			...
			<view
				...
				<uv-form-item :label="item.label" :name="item.key" :prop="item.key" borderBottom :required="item.required" v-if="item.showFunc ? item.showFunc() : true" v-bind="{ ...props.formItemProps, ...item.formItemProps }">
					...
					<view v-else-if="item.type === 'dateRange' || item.type === 'range'">
						<view @click="() => !(item.disabled || props.disabled) && proxy.$refs['range' + index][0].open()">
							<uv-input v-model="formData[item.key]" :placeholder="item.placeholder || '请选择'" clearable readonly border="surround" suffixIcon="arrow-down" :disabled="item.disabled" v-bind="{ ...item.props }"></uv-input>
						</view>
					...
					<view v-else-if="item.type === 'lookup' || item.type === 'lookupCode'" class="w-full">
						<view @click="() => !item.disabled && proxy.$refs['lookup' + index][0].showModal()">
							<uv-input class="pointer-events-none" v-model="formData[item.value || item.key + 'Text']" readonly :placeholder="item.placeholder || '请选择'" :disabled="item.disabled" clearable border="surround" suffixIcon="arrow-down"></uv-input>
						</view>
						<base-lookup-code
							:ref="'lookup' + index"
					...
					<view v-else-if="item.type === 'switch'" class="w-full">
						<uv-switch
							v-model="formData[item.key + 'Flag']"
							size="28"
							:disabled="item.disabled"
							@change="
								event => {
