 ...
 * @Author: guojiecheng
 * @LastEditTime: 2024-08-26 15:31:01
		...
		<uv-popup ref="popup" background-color="#fff" mode="right" safeAreaInsetTop>
			<view class="popup-content">
				<view class="signature-warp">
					<canvas canvas-id="canvas" type="2d" class="modal-canvas" disable-scroll="{{ true }}" id="canvas" @touchstart="handleTouchStart" @touchmove="handleTouchMove" @touchend="handleTouchEnd"></canvas>
					<view class="modal-title">签名板</view>
					<view class="modal-point">
						<view class="circle" :class="{ select: currentColor === item }" :style="{ 'background-color': item }" v-for="(item, index) in ['black', 'red', 'blue', 'green']" :key="index" @click="() => changeColor(item)"> </view>
					</view>
				...
				<canvas canvas-id="exportCanvas" type="2d" disable-scroll id="exportCanvas" style="position: fixed; top: -1000px; right: -1000px"></canvas>
			</view>
		</uv-popup>
	</view>
</template>
...
const exportSignature = (width, height, image) =>
	new Promise((resolve, reject) => {
